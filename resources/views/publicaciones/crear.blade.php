   ...
   <div class="container">
       @component('partials.info_formulario', ['rutaAtras' => route('publicaciones.index') ])
           ...
           @slot('informacionFormulario')
               Ingrese toda la información de la publicación.
           @endslot
       @endcomponent
       <form class="" action="{{ route('publicaciones.store') }}" method="post" enctype="multipart/form-data">
           @csrf

           <div class="form-group row">
               <label for="nombre" class="col-md-3 col-form-label text-md-left">
                   {{ __('Nombre de la publicación') }} <span class="text-danger">*</span>
           ...
           <div class="form-group row">
               ...
               <div class="col-md-6">
                   <textarea  id="descripcion" class="form-control{{ $errors->has('descripcion') ? ' is-invalid' : '' }}" name="descripcion" required>{{ old('descripcion') }}</textarea>
               </div>
           </div>
           ...
           <div class="form-group row">
               <label for="archivo" class="col-md-3 col-form-label text-md-left">
                   {{ __('Archivo (.pdf)') }} <span class="text-danger">*</span>
               ...
               <div class="col-md-6">
                   <input id="archivo" type="file" class="form-control{{ $errors->has('archivo') ? ' is-invalid' : '' }}" name="archivo" accept="application/pdf" required>
               </div>
           </div>
           ...
           <div class="form-group row">
                ...
                <div class="col-md-6">
                    <div class="custom-control custom-radio">
                        <input id="revista" type="radio" class="custom-control-input" name="tipo_publicacion" value="revista" {{ old('tipo_publicacion') == 'revista' ? 'checked' : '' }}>
                        <label class="custom-control-label" for="revista">
                            Revista
                        </label>
                    </div>
                    <div class="custom-control custom-radio">
                        <input id="boletin" type="radio" class="custom-control-input" name="tipo_publicacion" value="boletin" {{ old('tipo_publicacion') == 'boletin' ? 'checked' : '' }}>
                        <label class="custom-control-label" for="boletin">
                            Boletín
                        </label>
                    </div>
                    <div class="custom-control custom-radio">
                        <input id="libro" type="radio" class="custom-control-input" name="tipo_publicacion" value="libro" {{ old('tipo_publicacion') == 'libro' ? 'checked' : '' }}>
                        <label class="custom-control-label" for="libro">
                            Libro
                        </label>
                    </div>
                    <div class="custom-control custom-radio">
                        <input id="articulo" type="radio" class="custom-control-input" name="tipo_publicacion" value="articulo" {{ old('tipo_publicacion') == 'articulo' ? 'checked' : '' }}>
