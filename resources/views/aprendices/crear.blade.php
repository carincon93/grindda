...
@section('content')
    <div class="container">
        @component('partials.info_formulario', ['rutaAtras' => route('aprendices.index') ])
            @slot('tipoFormulario')
                Formulario para añadir un nuevo aprendiz.
            @endslot
            @slot('informacionFormulario')
                Ingrese toda la información del aprendiz.
            @endslot
        @endcomponent
        <form class="" action="{{ route('aprendices.store') }}" method="post" enctype="multipart/form-data">
            @csrf
                ...
                <div class="col-md-6">
                    <select id="programa_formacion_id" class="form-control{{ $errors->has('programa_formacion_id') ? ' is-invalid' : '' }}" name="programa_formacion_id" required>
                        ...
                        @foreach ($programasFormacion as $key => $programaFormacion)
                            <option value="{{ $programaFormacion->id }}" {{ old('programa_formacion_id') == $programaFormacion->id ? 'selected' : '' }}>{{ $programaFormacion->nombre }}</option>
                        @endforeach
                    </select>
