...
@section('content')
    <div class="container">
        @component('partials.info_formulario', ['rutaAtras' => route('carousel.index') ])
            ...
            @endslot
        @endcomponent
        <form class="" action="{{ route('carousel.update', $carouselItem->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            {{ method_field('PUT') }}

            <input type="hidden" name="tipoItem" value="{{ $carouselItem->tipo }}">
                    ...
                    <div class="col-md-6">
                        <select id="evento_id" class="form-control{{ $errors->has('evento_id') ? ' is-invalid' : '' }}" name="evento_id" required>
                            ...
                            @foreach ($eventos as $key => $evento)
                                <option value="{{ $evento->id }}" {{ $carouselItem->evento_id == $evento->id ? 'selected' : '' }}>{{ $evento->clasificacion }}</option>
                            @endforeach
                        </select>
            ...
            <div class="form-group row">
                ...
                <div class="col-md-6">
                    <input id="imagen" type="file" class="form-control{{ $errors->has('imagen') ? ' is-invalid' : '' }}" name="imagen" value="{{ old('imagen') }}" accept="image/*">
            ...
            @if ($carouselItem->tipo == 'otro')
                <div class="form-group row">
                    <label for="descripcion" class="col-md-3 col-form-label text-md-left">{{ __('Descripción') }} <span class="text-danger">*</span></label>

                    <div class="col-md-6">
                        <textarea id="descripcion" name="descripcion" class="form-control{{ $errors->has('descripcion') ? ' is-invalid' : '' }}" rows="8" cols="80" required>{{ $carouselItem->descripcion }}</textarea>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-6">
                        <input id="url" type="text" class="form-control{{ $errors->has('url') ? ' is-invalid' : '' }}" name="url" value="{{ $carouselItem->url }}" placeholder="http://google.com">
